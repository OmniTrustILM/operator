...
var _ = Describe("Connector Controller", func() {
	...
	Context("TestCreateConnectorBasic", func() {
		...
		It("should create ServiceAccount, Deployment, and Service with correct names, labels, and owner references", func() {
			...
			Eventually(func(g Gomega) {
				...
				g.Expect(sa.Labels).To(HaveKeyWithValue(connbuilder.NameLabel, connName))
				g.Expect(sa.Labels).To(HaveKeyWithValue(connbuilder.ManagedByLabel, connbuilder.ManagerName))
				g.Expect(sa.Labels).To(HaveKeyWithValue(connbuilder.ComponentLabel, connbuilder.ComponentValue))
			...
			Eventually(func(g Gomega) {
				...
				g.Expect(dep.Labels).To(HaveKeyWithValue(connbuilder.NameLabel, connName))
			...
			Eventually(func(g Gomega) {
				...
				g.Expect(svc.Labels).To(HaveKeyWithValue(connbuilder.NameLabel, connName))
	...
	Context("TestSecretChecksumChange", func() {
		...
		It("should update the Deployment checksum annotation when a referenced Secret changes", func() {
			...
			Eventually(func(g Gomega) {
				var dep appsv1.Deployment
				g.Expect(k8sClient.Get(ctx, key, &dep)).To(Succeed())
				cs, ok := dep.Spec.Template.Annotations[connbuilder.ChecksumAnnotation]
			...
			Eventually(func(g Gomega) {
				var dep appsv1.Deployment
				g.Expect(k8sClient.Get(ctx, key, &dep)).To(Succeed())
				cs := dep.Spec.Template.Annotations[connbuilder.ChecksumAnnotation]
	...
	// ---------------------------------------------------------------
	// leakyCoreBody is a distinctive token Core's error response might carry (a stack
	// trace, an internal hostname, a token). The registration client must surface only
	// the HTTP status code, so this must NEVER reach the Connector status condition /
	// event / logs (the "never in status/events/logs" invariant). Shared by the 4xx and
	// 5xx registration-failure specs.
	const leakyCoreBody = "internal-server-error-leaky-token-xyz789"
	...
	Context("TestRegistration5xxRetryable", func() {
		...
		It("should set Degraded condition with RegistrationFailed reason on 5xx", func() {
			By("starting an httptest server that returns 500 with a sensitive-looking body")
			server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
				w.WriteHeader(http.StatusInternalServerError)
				_, _ = w.Write([]byte(leakyCoreBody))
			}))
			defer server.Close()
			...
			By("verifying Degraded condition with RegistrationFailed reason and no body leak")
			Eventually(func(g Gomega) {
				var c otilmv1alpha1.Connector
				g.Expect(k8sClient.Get(ctx, key, &c)).To(Succeed())
				var found bool
				for _, cond := range c.Status.Conditions {
					if cond.Type == condDegraded && cond.Status == metav1.ConditionTrue {
						...
						// SECURITY: Core's raw response body must not flow into the condition.
						g.Expect(cond.Message).NotTo(ContainSubstring(leakyCoreBody),
							"Core response body must not leak into the status condition")
						g.Expect(cond.Message).To(ContainSubstring("status 500"))
						found = true
						break
					}
				}
	...
	Context("TestRegistration4xxNonRetryable", func() {
		...
		It("should set Degraded condition with RegistrationFailed reason on 4xx", func() {
			By("starting an httptest server that returns 400 with a sensitive-looking body")
			server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
			...
			Eventually(func(g Gomega) {
				...
				for _, cond := range c.Status.Conditions {
					if cond.Type == condDegraded && cond.Status == metav1.ConditionTrue {
						...
						g.Expect(cond.Message).NotTo(ContainSubstring(leakyCoreBody),
							"Core response body must not leak into the status condition")
						g.Expect(cond.Message).To(ContainSubstring("status 400"))
						found = true
						break
					}
				}
