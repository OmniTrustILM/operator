...
func TestRegisterSuccess(t *testing.T) {
	...
	resp, err := Register(context.Background(), client, &Request{
		Name:     testConnectorName,
...
func TestBuildRequest(t *testing.T) {
	...
	req := BuildRequest(testConnectorURL, reg)
	assert.Equal(t, testConnectorName, req.Name)
	assert.Equal(t, testAPIVersion, req.Version)
	assert.Equal(t, testConnectorURL, req.URL)
	assert.Equal(t, testAuthTypeBasic, req.AuthType)
...
func TestBuildRequestAuthNone(t *testing.T) {
	reg := &otilmv1alpha1.RegistrationSpec{
		Name:     "simple-connector",
		AuthType: otilmv1alpha1.AuthTypeNone,
	}

	req := BuildRequest("http://simple.default.svc.cluster.local:8080", reg)
