...
│   ├── manifests/                      # CSV, CRD, RBAC for OLM
│   ├── metadata/                       # OLM annotations
│   └── tests/scorecard/               # Scorecard test config
│
├── test/
│   ├── e2e/
...
│
├── hack/
│   └── sonar-local.sh                  # Local SonarQube analysis script
│
├── Dockerfile                          # Multi-stage: Go builder + distroless
├── bundle.Dockerfile                   # OLM bundle image Dockerfile
├── Makefile                            # Build, test, lint, kind, bundle, deploy
├── .golangci.yml                       # Linter configuration
├── .github/
│   └── workflows/
│       ├── test.yaml                   # Lint, test, build, E2E on PR/push
│       ├── dispatch-sonar.yaml         # Dispatches SonarCloud for PRs
│       ├── sonar.yaml                  # SonarCloud analysis with PR decoration
│       ├── publish_docker.yaml         # Build + push multi-arch Docker image
│       ├── test_docker_image.yaml      # Docker build + Trivy scan on PR
│       ├── publish_chart.yaml          # Helm chart publish to OCI registry
│       └── test_chart.yaml             # Helm chart lint + install test on PR
...
- `make bundle` — generate and validate the bundle
- `make bundle-build` / `make bundle-push` — build and push the bundle image
- `make catalog-build` / `make catalog-push` — build and push the catalog image

Install modes supported: OwnNamespace, SingleNamespace, AllNamespaces.
