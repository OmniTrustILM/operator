...
install: manifests kustomize ## Install CRDs into the K8s cluster specified in ~/.kube/config.
	# Server-side apply: the platform CRD exceeds the 256KiB last-applied-configuration
	# annotation that client-side apply writes, so a plain apply is rejected outright.
...
deploy: manifests kustomize ## Deploy controller to the K8s cluster specified in ~/.kube/config.
	cd config/manager && $(KUSTOMIZE) edit set image controller=${IMG}
	# Server-side apply: config/default carries the platform CRD, which exceeds the 256KiB
